...
const UserProfile = () => {
  ...
  const UserForm = ({ open, setOpen }) => {
    ...
    const onSubmit = async (data) => {
      ...
      // Close the form immediately when the submit button is clicked
      setOpen(false);
      try {
        ...
        if (res) {
          const updatedUserInfo = {
            ...user, // Keep token and other fields unchanged
            ...res.user, // Only update the fields returned by the response (e.g., profile)
          };
          dispatch(Login(updatedUserInfo));
          setProfileUpdated(true);
           // Trigger refetch in UserProfile
        }
  ...
  const fetchUserData = async () => {
    try {
      const res = await apiRequest({
        url: "/users/get-user", // Ensure this matches your backend route
        token: user?.token,
        method: "GET",
      });
      if (res.success) {
  ...
  // Fetch user data when the profile is updated
  useEffect(() => {
    if (profileUpdated) {
      fetchUserData();
      setProfileUpdated(false); // Reset the update state
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [profileUpdated]); // Only re-fetch when profileUpdated changes
  ...
  return (
    <div className='container mx-auto flex items-center justify-center py-10'>
      ...
      {/* Pass setProfileUpdated to trigger refetch when profile is updated */}
      <UserForm open={open} setOpen={setOpen} setProfileUpdated={setProfileUpdated} />
    </div>
  );
};
